...
> [!NOTE]
> Prior to `v1.3.0.0` of Chaos Mage the lists used a different versioning scheme and structure for its updates. Since that release it now follows the structure of `MAJOR`.`MINOR`.`PATCH`.`REVISION` and the meaning of the numbers in each section has been standardized. Check the [Readme](https://github.com/OmniWildcard/Omni-s_World/blob/main/Readme_Files/CMRC_Readme.md#updating-the-modlist) for more information on what each kind of update entails.
